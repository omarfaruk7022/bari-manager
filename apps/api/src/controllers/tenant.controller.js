...
export const create = async (req, res, next) => {
  try {
    ...
    if (req.body.email) {
      ...
      await tenant.save()
      await sendCredentialsEmail({ name: req.body.name, email: req.body.email, password: tempPassword })
    }
    ...
    res.status(201).json({ success: true, message: 'ভাড়াটে যুক্ত হয়েছে', data: tenant })
  } catch (err) { next(err) }
}
