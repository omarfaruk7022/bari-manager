...
export const create = async (req, res, next) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "খরচ যুক্ত হয়েছে", data: expense });
  } catch (err) {
    next(err);
  }
};
...
export const update = async (req, res, next) => {
  try {
    ...
    const expense = await Expense.findOneAndUpdate(filter, req.body, {
      new: true,
    });
    if (!expense)
      return res
        .status(404)
        .json({ success: false, message: "খরচ পাওয়া যায়নি" });
...
export const splitToBills = async (req, res, next) => {
  try {
    ...
    if (!expense.month)
      return res
        .status(400)
        .json({ success: false, message: "মাস উল্লেখ নেই" });
    ...
    res.json({
      success: true,
      message: `${splits.length}টি বিলে যুক্ত হয়েছে`,
    });
  } catch (err) {
    next(err);
  }
