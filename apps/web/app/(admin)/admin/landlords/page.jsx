  ...
  return (
    <div className="py-4 space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {landlords.map(l => (
            <div key={l._id} className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {l.profile?.propertyName && <p className="text-sm text-gray-500">{l.profile.propertyName}</p>}
                  <p className="text-xs text-gray-500 mt-1">সক্রিয় ভাড়াটে: {l.activeTenants || 0}</p>
              ...
              {editing === l._id ? (
                ...
                >
                  <input name="name" defaultValue={l.name} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input name="email" defaultValue={l.email} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input name="phone" defaultValue={l.phone} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input name="propertyName" defaultValue={l.profile?.propertyName} className="rounded-lg border border-gray-200 px-3 py-2" />
                  <input name="propertyAddress" defaultValue={l.profile?.propertyAddress} className="rounded-lg border border-gray-200 px-3 py-2 md:col-span-2" />
                  <input name="totalUnits" type="number" defaultValue={l.profile?.totalUnits || 0} className="rounded-lg border border-gray-200 px-3 py-2" />
                ...
                <div className="flex gap-2 border-t border-gray-100 pt-4">
                  <button onClick={() => setEditing(l._id)} className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700">
                    <Edit3 size={16} /> সম্পাদনা
                  </button>
                  <button
                    onClick={() => {
                      if (!confirm('এই ল্যান্ডলর্ড ও সম্পর্কিত তথ্য মুছবেন?')) return
