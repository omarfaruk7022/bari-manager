      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {subs.map(s => {
            ...
            return (
              <div key={s._id} className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
                ...
                {s.propertyName && (
                  <div className="bg-gray-50 rounded-xl p-3 text-sm text-gray-600">
                    <p><strong>বাড়ির নাম:</strong> {s.propertyName}</p>
                    {s.propertyAddress && <p><strong>ঠিকানা:</strong> {s.propertyAddress}</p>}
                    {s.totalUnits && <p><strong>ফ্ল্যাট সংখ্যা:</strong> {s.totalUnits}</p>}
                  </div>
                )}
                ...
                <p className="text-xs text-gray-400">
                  আবেদনের তারিখ: {new Date(s.createdAt).toLocaleDateString('bn-BD')}
                </p>
                ...
                {s.status === 'pending' && (
                  <div className="flex gap-3 pt-1">
                    ...
                    >
                      {acting === s._id + '-reject' ? 'প্রক্রিয়াধীন...' : 'প্রত্যাখ্যান'}
                    </button>
                    <button
                    ...
                    >
                      {acting === s._id + '-approve' ? 'প্রক্রিয়াধীন...' : 'অনুমোদন করুন'}
                    </button>
                  </div>
                )}
              </div>
